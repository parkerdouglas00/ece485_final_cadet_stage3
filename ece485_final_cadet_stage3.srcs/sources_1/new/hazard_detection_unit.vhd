...
begin
    ...
    process(if_id_mem_read, if_id_rd, rs1, rs2, if_id_opcode, opcode, stall_counter, opcode, stall_counter, reset)
    begin      
        if (reset = '1') then
            start_stall <= '0';
        -- stall cases, dependency on a (1)load from memory, (2) load_addr, (3) add, (4) addi/subi
        elsif ((if_id_rd /= "00000") and
           ((if_id_mem_read = '1') or (if_id_load_addr = '1') or
           (if_id_opcode = "0110011") or (if_id_opcode = "0010011")) and
           ((if_id_rd = rs1) or (if_id_rd = rs2)) and
           (stall_counter = 0)) then 
 -- single stall data dependency case
                start_stall <= '1';
        elsif -- stall cases for branch or jump, needing time to calulate branch address, etc
              ((opcode = "1100011" or opcode = "1101111") and (stall_counter = 0)) then 
                start_stall <= '1';      
        else        
                start_stall <= '0';
        end if;    
